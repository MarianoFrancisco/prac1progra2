<%-- 
    Document   : ordenar_mayor_fecha_mueble
    Created on : 4/09/2021, 18:06:09
    Author     : Mariano
--%>
...
<html>
    <head>
        ...
        <title>Ordenar mayor fecha</title>
    </head>
    <body>
        <%
            String usuarioInicial=request.getParameter("usuarioInicial");
        %>
        <div id="titulo"><h1>Consulta Mueble Mayor a Menor Fecha</h1></div>
        <hr>
        <%
            Connection cn=null;
            try{
        ...
        <tbody>
            <%
                try{
                    ...
                    rs=st.executeQuery("select nombre_mueble,nombre_usuario,precio,fecha from ensamblar order by fecha DESC");
            while(rs.next()){
                %>
                <tr>
